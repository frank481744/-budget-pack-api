...
async function sendPushFilters(env,filters,title,message,url){
  if(!env.ONESIGNAL_APP_ID||!env.ONESIGNAL_REST_API_KEY||!filters?.length)return false;
  const r=await fetch("https://api.onesignal.com/notifications",{method:"POST",headers:{"content-type":"application/json","authorization":`Key ${env.ONESIGNAL_REST_API_KEY}`},body:JSON.stringify({app_id:env.ONESIGNAL_APP_ID,filters,target_channel:"push",headings:{fr:title,en:title},contents:{fr:message,en:message},url})});
...
export default {
  async fetch(req,env){
    ...
    try{
      ...
      if(path==="/api/push-config"&&req.method==="GET")return json({appId:env.ONESIGNAL_APP_ID||""});
